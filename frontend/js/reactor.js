...
function openModal(type) {
  ...
  modalPercent.placeholder = type === 'charge' ? 'How much closer to your dream? (%)' : 'How much further from it? (%)';
...
modalPercent.addEventListener('keydown', e => { if (e.key === 'Enter') submitEntry(); });
modalSubmit.addEventListener('click', submitEntry);

async function submitEntry() {
  const text = modalInput.value.trim();
  const weight = Number(modalPercent.value);

  if (!text) { modalInput.focus(); return; }
  if (!weight || Number.isNaN(weight) || weight <= 0) {
    showToast('Enter how much this changes things (%)', 'error');
    modalPercent.focus();
    return;
  }
  ...
  try {
    const { entry, charge: newCharge, leaks, race, raceResolved } = await api.post('/reactor', { type, text, weight });
...
// after that entry — cyan/rising for a charge, red/falling for a leak.
// Each entry's height contribution is whatever percent you typed in when
// logging it, not a fixed step — so a huge, especially impactful entry
// can push the running total past 100%, and the axis stretches to fit it.
function renderTodayChart() {
  ...
  const points = sorted.map(e => {
    ...
    running = e.kind === 'charge' ? running + weight : Math.max(0, running - weight);
    return { kind: e.kind, value: running };
  });

  // Axis always shows at least 0→125 (the old default headroom), but
  // stretches further — in steps of 25 — if today's peak goes past that.
  const peak = Math.max(...points.map(p => p.value), 100);
  const axisMax = Math.max(125, Math.ceil(peak / 25) * 25 + 25);
  const tickStep = axisMax <= 200 ? 25 : Math.ceil(axisMax / 8 / 25) * 25;
  const ticks = [];
  for (let t = tickStep; t < axisMax; t += tickStep) ticks.push(t);

  yAxisEl.innerHTML = `<span class="headroom">${axisMax}</span>` +
    ticks.slice().reverse().map(t => `<span>${t}</span>`).join('');

  gridEl.innerHTML = [...ticks, axisMax].map(t =>
    `<div class="gridLine ${t === axisMax ? 'headroom' : ''}" style="bottom:${(t / axisMax) * 100}%"></div>`
  ).join('');

  barsEl.innerHTML = points.map((p, i) => `
    <div class="barItem">
      <span class="barPct">${Math.round(p.value)}%</span>
      <div class="barShape ${p.kind === 'charge' ? 'up' : 'down'}" style="height:${(p.value / axisMax) * 100}%"></div>
